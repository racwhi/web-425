...
<label>Rate our service</label> @for(rating of ratings; track rating) {
<input type="radio" [value]="rating" formControlName="rating"> {{ rating }}
<br />
}
...
@for(option of recommendedOptions; track option) { <option [value]="option">{{ option }}</option>
}
</select>
...
@for(feedback of preexistingFeedback; track feedback) { <div class="feedback-card">
<h2>Rating: {{ feedback.rating }}</h2>
<h3>Likes:</h3>
<ul class="likes-list">
@for(like of feedback.likes; track like) { <li>{{ like }}</li>
}
</ul>
...
recommendedOptions: string[] = ['Yes', 'No']; preexistingFeedback: any;

feedbackForm: FormGroup = this.fb.group({ rating: [null,
